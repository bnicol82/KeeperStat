...
- Production migrations run automatically inside **Vercel's build** (`vercel.json`'s `buildCommand` runs `db:migrate` before `build`, using the `DATABASE_URL` the Neon integration injects). `db/migrate.js` is idempotent, and a failing migration fails the build, so a deploy can never outrun its schema. `.github/workflows/migrate-production.yml` remains as a manual fallback (Actions tab → Run workflow) for migrating without deploying; it requires a `PRODUCTION_DATABASE_URL` repository secret (Settings → Secrets and variables → Actions) and validates the secret's shape before connecting.
